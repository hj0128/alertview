...
# guess_in 용 어휘(텍스트에 '글자 그대로' 있으면 그 카테고리로 본다).
# '포장/배달/체험'은 '포장마차·배달불가·체험단' 등에 오탐하므로 제외 — 그것들은 classify 가 키워드로 처리.
CATEGORIES = ["맛집", "카페", "뷰티", "건강", "숙박", "문화", "스포츠",
              "생활", "편의", "배송", "페이백", "기자단"]
...
class BaseAdapter:
    ...
    prunable: bool = False
    # 이 어댑터만의 최소 수집 간격(초). 0 이면 매 폴링. (예: 캠페인이 매우 많아 자주 안 돌려도 되는 사이트)
    min_interval: int = 0
