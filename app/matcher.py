...
CATEGORY_KEYWORDS = {
    # 순서 = 우선순위(앞이 먼저 매칭). 구체/명시적인 것을 앞에 둔다.
    # 포장(테이크아웃)은 맛집보다 우선. '포장마차' 등에 안 걸리게 '포장 ~' 조합어만.
    "포장": ["테이크아웃", "테이크 아웃", "포장 체험", "포장체험", "포장 가능", "포장가능",
           "포장 주문", "포장주문", "포장 전용", "포장전용", "포장 할인", "포장 메뉴", "포장만"],
    # 배달앱 명시(배민·요기요 등) 또는 '[배달]'·'배달체험'만. '배달불가' 등 오탐 방지.
    "배달": ["배달앱", "배달의민족", "배민", "요기요", "쿠팡이츠", "배달체험", "[배달]"],
    "페이백": ["페이백", "캐시백", "리워드", "적립", "페이 백"],
    "기자단": ["기자단"],
    "숙박": ["숙박", "호텔", "펜션", "리조트", "풀빌라", "글램핑", "오토캠핑", "캠핑", "게스트하우스",
           "모텔", "스테이", "카라반", "독채", "민박", "한옥스테이", "료칸", "1박"],
    "카페": ["카페", "커피", "디저트", "베이커리", "케이크", "케익", "빵집", "제과", "마카롱",
           "브런치", "와플", "빙수", "티룸", "쿠키", "도넛", "아이스크림", "젤라또", "휘낭시에",
           "까눌레", "스무디", "베이글", "크로플", "약과", "다과", "에이드", "라떼", "디저트카페"],
    "뷰티": ["뷰티", "미용실", "미용", "헤어", "염색", "네일", "왁싱", "속눈썹", "래쉬", "메이크업",
           ...
           "태닝", "제모", "두피케어", "헤어케어", "속눈썹펌", "왁스"],
    "건강": ["헬스", "피트니스", "PT", "필라테스", "요가", "다이어트", "체형", "교정", "도수",
           "한의원", "한방", "병원", "의원", "치과", "영양제", "건강기능식품", "유산균", "비타민",
           "마사지", "스파", "경락", "사우나", "찜질", "림프", "바디케어", "아로마", "탈모",
           "두피", "스트레칭", "재활", "체형교정", "다이어트관리", "마사지샵", "바레", "교정원", "물리치료"],
    "스포츠": ["골프", "스크린골프", "볼링", "당구", "클라이밍", "서핑", "테니스", "수영", "배드민턴",
           "풋살", "축구", "야구", "농구", "스쿼시", "사격", "양궁", "승마", "스키", "스노보드",
           "요트", "카약", "러닝", "사이클", "복싱", "주짓수", "크로스핏", "암벽", "스케이트", "유도", "검도", "태권도", "합기도", "무에타이", "펜싱", "탁구"],
    "문화": ["공연", "전시", "뮤지컬", "연극", "콘서트", "영화", "박물관", "미술관", "갤러리",
           "공방", "원데이", "클래스", "도자기", "드로잉", "악기", "레슨", "전시회", "페스티벌", "보컬", "댄스", "발레", "피아노", "바이올린", "드럼", "음악학원", "미술학원", "노래교실"],
    "체험": ["키즈카페", "액티비티", "나들이", "투어", "놀이공원", "테마파크", "아쿠아리움",
           "동물원", "실내놀이터", "트램폴린", "VR", "방방", "키즈", "원데이체험", "농장체험",
           "수목원", "글램핑카페", "방탈출", "방탈", "보드게임",
           # 사진/스튜디오('사진'·'촬영'은 후기 캠페인에 흔해 오탐 → 사진관·프로필 등 명확한 표현만)
    ...
    "맛집": ["맛집", "식당", "레스토랑", "다이닝", "오마카세", "디너", "코스요리", "한우", "고기",
           ...
           "막창", "대창", "곱창전골", "장어", "낙지", "해물", "조개", "덮밥", "비빔밥", "만두",
           "야키토리", "꼬치", "텐동", "규동", "초밥집", "포케", "쌈밥", "두부", "감자탕",
           "순대", "수육", "마라탕", "샤브", "전집", "식사권", "외식", "한끼", "맛집체험"],
    "생활": ["생활용품", "청소", "세탁", "인테리어", "가구", "리빙", "주방용품", "수납",
           "반려", "강아지", "고양이", "애견", "반려견", "반려묘", "펫", "꽃다발", "꽃집",
           "원예", "화분", "방향제", "디퓨저", "가전", "조명", "침구", "커튼", "청소기", "세제",
           # 개인 생활서비스(운세·소개팅 등 — 마땅한 분류가 없어 생활로)
           "타로", "사주", "점집", "신점", "운세", "소개팅", "결혼정보", "매칭",
           # 꽃/청소/홈서비스
           "플라워", "꽃다발", "다발", "화훼", "홈케어", "홈클리닝", "입주청소", "청소대행", "정리수납", "재무상담", "재무설계", "보험상담"],
    "편의": ["편의점", "마트", "주유", "세차", "렌탈", "렌터카", "수리", "이사", "대행",
           "구독", "통신", "핸드폰", "휴대폰", "보험", "공유오피스", "스터디카페", "프린트", "타이어", "카센터", "정비", "세탁소"],
    "배송": ["배송", "택배", "제품", "식품", "집으로"],
}
